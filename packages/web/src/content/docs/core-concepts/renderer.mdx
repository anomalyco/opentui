...
Set `nativeSceneWorkBudget` to a positive unsigned 32-bit integer to yield during frame preparation. It counts preparation visits, view-preparation entries, and feedback records between yields. If a yield accepts a layout change, preparation restarts once and completes without further yields.
