...
Set `nativeSceneWorkBudget` to a positive unsigned 32-bit integer to yield during frame preparation. It counts preparation visits, view-preparation entries, and feedback records between yields. If a yield accepts a layout change, preparation restarts once, and the rest of the frame runs without yields.

The quota does not bound elapsed time or cells. Yoga, sibling sorting, final publication, painting, and output encoding still run synchronously. Painting never yields, so a frame never mixes cells from before and after a mutation. The scheduler also cannot preempt an application callback.

An accepted changed-size resize supersedes a draft parked at a scheduling yield. The replacement frame keeps accepted mutations and callback side effects; cancellation is not rollback. Output already admitted retains its ordering and completion accounting.
