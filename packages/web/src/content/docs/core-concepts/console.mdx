...
Each overlay entry contains a wall-clock `Date`, a log level, and the original argument array. Display formatting follows `util.format()`, the same policy `console.log` uses: a leading string substitutes format specifiers such as `%s`, `%d`, and `%j`, top-level strings print raw with their escapes interpreted, and other values are inspected at depth `2`. Info, warning, error, debug, and default log entries use separate configured colors.
