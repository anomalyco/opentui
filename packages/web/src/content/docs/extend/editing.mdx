...
Read the native [copy rules](/docs/native/resources#read-text-and-geometry) for output units and capacity failures.
...
```typescript example=shared-text
...
```text example=shared-text-output
Hello
```

Only the resource-backed path permits direct updates through the protected buffer.
Call `updateTextInfo()` after those updates to refresh selection and request rendering.
The [integration description](/docs/extend/custom-renderables#declare-native-integration) controls native drawing and host hooks.
