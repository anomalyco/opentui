...
- Native scene painting and post-process callbacks draw into `nextRenderBuffer`.
- Paint hooks receive a separate recording buffer. Its drawing calls record commands that native code plays when it paints `nextRenderBuffer`. It cannot read cells.
...
A paint hook's buffer starts each hook with the node's clipping and opacity. Its pop
and clear calls affect only its own entries, not inherited state. Custom entries reset
between hooks. The buffer records drawing only while its frame runs hooks, so a saved
hook buffer cannot draw later.
