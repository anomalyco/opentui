...
pub const TextViewCommand = union(enum) {
    ...
    tab_indicator: ?u32,
    truncate: bool,
    text_align: text_buffer_view.TextAlign,
};
...
pub const Context = struct {
    ...
    /// Reads one current-cell link id without retaining storage. Encode syncs
    /// those cells before presentation is pending, so this stays valid while
    /// output drains. Out-of-range coordinates are not a link.
    pub fn sessionGetLinkId(self: *Context, handle: Handle, x: i32, y: i32) !u32 {
        try self.beginMutation();
        defer self.mutating = false;
        const value = try self.getSession(handle);
        ...
        const cell = attached.getCurrentBuffer().get(@intCast(x), @intCast(y)) orelse return 0;
    ...
    pub fn embeddedTerminalCommand(self: *Context, handle: Handle, command: u32, argument: i32) !void {
        ...
        const value = try self.getEmbeddedTerminal(handle);
        switch (command) {
            api.OT_EMBEDDED_TERMINAL_INVALIDATE => {
                if (argument != 0) return error.InvalidOptions;
                value.invalidate();
            },
            api.OT_EMBEDDED_TERMINAL_SCROLL => value.scroll(argument),
            api.OT_EMBEDDED_TERMINAL_CLEAR_SELECTION => {
                if (argument != 0) return error.InvalidOptions;
                value.clearSelection();
            },
            api.OT_EMBEDDED_TERMINAL_SET_TRANSPARENT_BACKGROUND => {
                if (argument != 0 and argument != 1) return error.InvalidOptions;
                value.setTransparentBackground(argument == 1);
            },
    ...
    pub fn sceneSetEditorOptions(self: *Context, handle: Handle, options: scene.EditorOptions) !void {
        ...
        const node = value.scene_node.?;
        if (node.kind != api.OT_SCENE_EDITOR) return error.WrongKind;
        if (options.style > 3 or (options.mouse_pointer > api.OT_MOUSE_POINTER_MAX and
            options.mouse_pointer != api.OT_MOUSE_POINTER_UNCHANGED)) return error.InvalidOptions;
    ...
    pub fn sceneSetTextOptions(self: *Context, handle: Handle, options: scene.TextOptions) !void {
        ...
        text.view.setTextAlign(options.text_align);
    ...
    pub fn textViewCommand(self: *Context, handle: Handle, command: TextViewCommand) !void {
        ...
        switch (command) {
            ...
            .text_align => |alignment| view.view.setTextAlign(alignment),
        }
        view.invalidate();
    }
