...
static void rendered_output(ot_context *context) {
    ...
    assert(ot_scene_paint(context, &session, background, 1, 0, NULL) == OT_INVALID_ARGUMENT);
    ...
    assert(ot_scene_frame_commit(context, &session, &frame, 2, &result) == OT_INVALID_ARGUMENT);
    assert(result == UINT32_MAX);
    assert(ot_scene_frame_commit(context, &session, &frame, 1, &result) == OT_OK && result == OT_RENDER_PENDING);
    assert(ot_scene_frame_commit(context, &session, &frame, 1, &result) == OT_STALE_FRAME);
    const ot_scene_frame_request previous = frame;
    assert(ot_scene_paint(context, &session, background, 1, 0, &frame) == OT_OUTPUT_BUSY);
